	...
	<UCard>
		<div class="flex flex-col gap-6">
			<div class="flex items-center justify-between">
				...
				<UBadge :color="getStatusColor(invoice?.status)" size="lg" variant="soft">
					{{ invoice?.status }}
				</UBadge>
			</div>
			...
			<div v-if="invoice?.notes" class="mt-6">
				<UCard>
					...
					<p class="text-toned">{{ invoice.notes }}</p>
				</UCard>
			</div>
...
const { data: invoice } = await useFetch<InvoiceWithProject>(
	`/api/invoices/${invoiceId}`,
);
const { data: invoiceItems } = await useFetch<InvoiceItem[]>(
	`/api/invoices/${invoiceId}/items`,
);
const { data: payments } = await useFetch<Payment[]>(
