...
const {
	...
	pending,
	error,
} = await useFetch<Project & { client: { id: string; name: string } }>(
...
const state = reactive({
	...
	rateType: "HOURLY" as "HOURLY" | "FIXED" | "DAILY",
	rateAmount: undefined as number | undefined | null,
	currency: "GBP",
...
watchEffect(() => {
	if (project.value) {
		...
		state.startDate = project.value.startDate
			? new Date(project.value.startDate).toISOString().split("T")[0]
			: "";
		state.endDate = project.value.endDate
			? new Date(project.value.endDate).toISOString().split("T")[0]
			: "";
